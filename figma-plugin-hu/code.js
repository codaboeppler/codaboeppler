...
function detectInteraction(node) {
...
function isErrorColor(r, g, b) {
    ...
    if (r > 0.8 && g < 0.4 && b < 0.2)
        return true;
    return false;
}
// Extrae colores de fills y strokes de un nodo
function extractColors(node) {
    const result = { fills: [], strokes: [], hasRedFill: false, hasRedStroke: false };
    if ('fills' in node && Array.isArray(node.fills)) {
        for (const fill of node.fills) {
            if (fill.type === 'SOLID' && fill.visible !== false) {
                ...
                if (isErrorColor(r, g, b))
                    result.hasRedFill = true;
            }
        }
    }
...
function detectInputErrors(node, parentName = '') {
    const errors = [];
    const nameLower = node.name.toLowerCase();
    ...
    if (node.type === 'TEXT') {
        ...
        if (errorTextPatterns.some(p => text.includes(p))) {
            errors.push({
                fieldName: contextName,
                errorType: 'text',
                detail: `Texto de error detectado: "${node.characters.trim()}"`
            });
        }
        // Texto rojo tambien es indicador de error
        const textColors = extractColors(node);
        if (textColors.hasRedFill && node.characters.trim().length > 0) {
            errors.push({
                fieldName: contextName,
                errorType: 'color',
                detail: `Texto en rojo: "${node.characters.trim()}" - posible mensaje de error`
            });
        }
    }
        ...
        try {
            ...
            if (props) {
                for (const [key, val] of Object.entries(props)) {
                    ...
                    if ((keyLower.includes('error') && (valStr === 'true' || valStr === 'yes')) ||
...
function categorizeComponents(info) {
    const result = {
        inputs: [],
        buttons: [],
        labels: [],
    ...
    function walk(node, parentIsInput = false) {
        const nameLower = node.name.toLowerCase();
    ...
    function collectErrors(node, parentFieldName) {
        if (node.hasErrorState) {
            // Re-tag errors with parent field name
            for (const err of node.errorDetails) {
                result.inputErrors.push(Object.assign(Object.assign({}, err), { fieldName: parentFieldName }));
            }
        }
        for (const child of node.children) {
...
function generateHU(info, screenName, huNumber, huType, context) {
    ...
    if (categories.indicators.length > 0) {
        ...
        categories.indicators.forEach((ind, idx) => {
            ...
            if (ind.texts.length > 0)
                hu += ` - muestra: "${ind.texts[0]}"`;
            hu += '\n';
        });
    }
...
async function exportFrameAsPng(node) {
    try {
...
figma.ui.onmessage = async (msg) => {
    if (msg.type === 'generate') {
        ...
        const hu = generateHU(info, msg.screenName || node.name, msg.huNumber || 'XXX', msg.huType || 'screen', msg.context || '');
        figma.ui.postMessage({
            type: 'result',
            hu: hu,
            nodeName: node.name
